...
export function NavMain({ items }: NavMainProps) {
  ...
  useEffect(() => {
    ...
    checkRole();
    window.addEventListener("atcis-auth-changed", checkRole);
    return () => window.removeEventListener("atcis-auth-changed", checkRole);
  }, []);
  ...
  // Roles permitted to create/log tenders directly
  const canQuickCreate = ["super_admin", "country_admin", "hod", "account_manager"].includes(currentUserRole);
  ...
  return (
    <>
      ...
      <LogManualTenderDialog open={isCreateOpen} onOpenChange={setIsCreateOpen} />
      {items.map((group) => {
        ...
        // Hide group if no items are visible for this role
